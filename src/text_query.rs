...
fn parse_term(iter: &mut Iter) -> Expr {
    match iter.next().unwrap() {
        ...
        Token::Hex(num) =>
            ...
            .map(Num::Mpq)
            .map(Expr::Const)
            .unwrap_or_else(|()| Expr::Error("Failed to parse hex".to_string())),
        Token::Oct(num) =>
            Mpz::from_str_radix(&*num, 8)
            .map(|x| Mpq::ratio(&x, &Mpz::one()))
            .map(Num::Mpq)
            .map(Expr::Const)
            .unwrap_or_else(|()| Expr::Error("Failed to parse octal".to_string())),
        Token::Bin(num) =>
            Mpz::from_str_radix(&*num, 2)
            .map(|x| Mpq::ratio(&x, &Mpz::one()))
            .map(Num::Mpq)
            .map(Expr::Const)
            .unwrap_or_else(|()| Expr::Error("Failed to parse binary".to_string())),
